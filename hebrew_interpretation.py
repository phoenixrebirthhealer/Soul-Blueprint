...
def build_batch_prompt(questionnaire_items):
    ...
    return (
        "You are a sacred depth reader for the Hebrew Metatron's Cube Frequency System by Phoenix Rebirth.\n\n"
        "Each Hebrew letter holds an archetypal energy field with two expressions: its highest gift and its "
        "contracted shadow. A person's written reflection about their experience with that letter reveals which "
        "expression they are currently living from.\n\n"
        "Your role is to read the resonance between each reflection and its letter's energy field, then name "
        "where this person is in that arc. This is not categorization. This is sacred discernment — reading "
        "what the words carry beneath what they say.\n\n"
        "Resonance definitions:\n"
        "- \"healed\": The reflection carries the energy of the archetype's highest gift. Integration, flow, "
        "ownership, ease, or embodied knowing of this energy is present.\n"
        "- \"shadow\": The reflection carries the archetype's contracted energy. Resistance, avoidance, grief, "
        "disconnection, or unfamiliarity with this energy is present.\n"
        "- \"bridge\": The reflection holds both expressions at once. This person is in active transition — "
        "aware of both the wound and the gift, moving between them.\n\n"
        "Read each position below and name where this person stands in that archetype's arc:\n\n"
        "---\n"
        + "\n---\n".join(blocks)
        + "\n\n"
        "Return ONLY a valid JSON object mapping position numbers (as strings) to resonance status words.\n"
